						...
						do {
							...
							if (!found) {
								if (r1.getArrQuestion().get(numQue) instanceof CloseQuestions) {
									que = CloseQuestionsFactory.create();
									que.setQuestionFromQustion(r1.getArrQuestion().get(numQue));
									haveTrue = false;
									countAns = 0;
									do {
										correct = false;
										...
										answer.setAnswerToQuestion(r1.getArrAnswer().get(numAns), correct);
									...
									que = OpenQuestionFactory.create();
									que.setQuestionFromQustion(r1.getArrQuestion().get(numQue));
								}
